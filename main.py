...
class Project():
    def __init__(self):
        ...
        self.test_img, self.test_lbl = util.load_mnist(self.labelN, self.lbl_n)
    ...
    def Lcclp(self, sup_lbl_onehot, feat, cclp_steps=3):
        # ---------- Perform Label Propagation (LP) ---------------
        phi_L = sup_lbl_onehot
        # Graph's affinity mtx. [N,F] x [F,N] = [N, N]
        ...
        H_uu = H_trans[n_split:, n_split:]  # Eq.4. Transition probabilities between Unlabelled samples only.
        H_ul = H_trans[n_split:, :n_split]  # Eq.4. Transition probabilities from Unlabelled to Labelled samples only.
        ...
        mass_per_c = tf.reduce_sum(phi, axis=0)  # Mass of each class. Matrix [ 1, C_classes ]
        ...
        phi_div_mass = phi / (mass_per_c + 1e-5)  # elem wise after broadcasting mass_per_c. Returns [NxC]. Mass >> eps.
        ...
        loss = 0  # Will hold final result, Eq.9
        H_step = H_trans  # H_step will hold H_mtx^(s) of Eq.8.
        eps_log = 1e-6
        ...
        M_class_match = tf.matmul(phi, phi, transpose_b=True)  # [NxN] mtx. Dot prod, 0-1, confidence that two samples are of same class.
        H_Masked = H_trans * M_class_match  # Eq.8. M can be seen as a soft mask applied to H_mtx, allowing steps only between same-class samples.
    ...
    def train_step(self, sup_img, sup_lbl, unsup_img):
        with tf.GradientTape() as tape:
            sup_feat, sup_pred = self.model(sup_img)
            unsup_feat, _ = self.model(unsup_img)
            ...
            cclp_loss, _, H, _, _, _, _, _, _, _, _, phi_U = self.Lcclp(sup_lbl_onehot, feat)
