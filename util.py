...
def load_mnist(labelN, lbl_n):
    ...
    # split sup and unsup
    sup_img, sup_lbl = get_sup_batch(x_train, y_train, C_index_lst, labelN, lbl_n)  # (100, 28, 28, 1) (100,)
    unsup_img, unsup_lbl = get_unsup_batch(x_train, y_train, C_index_lst, labelN, lbl_n)  # (541, 100, 28, 28, 1) (541, 100)

    # get test index
    C_index_lst_ = get_index_per_class(y_test, labelN, lbl_n)

    # split val and test
    val_img, val_lbl = get_sup_batch(x_test, y_test, C_index_lst_, labelN,  lbl_n)  # (100, 28, 28, 1) (100,)
    test_img, test_lbl = get_unsup_batch(x_test, y_test, C_index_lst_, labelN, lbl_n)  # (88, 100, 28, 28, 1) (88, 100)
